...
public class SendBuddyMessageResult implements org.apache.thrift.TBase<SendBuddyMessageResult, SendBuddyMessageResult._Fields>, java.io.Serializable, Cloneable, Comparable<SendBuddyMessageResult> {
  ...
  private BuddyResultState state; // required
  private String messageId; // required
  private int eventNo; // required
  private long receiverCount; // required
  private long successCount; // required
  private long failCount; // required
  private long cancelCount; // required
  private long blockCount; // required
  private long unregisterCount; // required
  ...
  public int hashCode() {
    ...
    boolean present_eventNo = true;
    list.add(present_eventNo);
    if (present_eventNo)
      list.add(eventNo);

    boolean present_receiverCount = true;
    list.add(present_receiverCount);
    if (present_receiverCount)
      list.add(receiverCount);
    ...
    boolean present_failCount = true;
    list.add(present_failCount);
    if (present_failCount)
      list.add(failCount);

    boolean present_cancelCount = true;
    list.add(present_cancelCount);
    if (present_cancelCount)
      list.add(cancelCount);

    boolean present_blockCount = true;
    list.add(present_blockCount);
    if (present_blockCount)
      list.add(blockCount);

    boolean present_unregisterCount = true;
    list.add(present_unregisterCount);
    if (present_unregisterCount)
      list.add(unregisterCount);
        ...
        switch (schemeField.id) {
          ...
          case 2: // STATE
            if (schemeField.type == org.apache.thrift.protocol.TType.I32) {
              struct.state = com.linecorp.thrift.BuddyResultState.findByValue(iprot.readI32());
